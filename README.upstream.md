...
> ### ⚡ Community modernization fork
>
> This is a **community-maintained fork** of the original [GitSquared/edex-ui](https://github.com/GitSquared/edex-ui),
> which was archived on Oct 18 2021. It is **not affiliated with or endorsed by** the original author.
> All credit for eDEX-UI goes to Gabriel "Squared" Saillard and the original contributors.
>
> **What this fork does:** revives the project on a modern stack so it builds and runs on
> current operating systems (including **Apple Silicon**). The 2021 toolchain was bumped:
> Electron 12 → 33, `node-pty` 0.10 → 1.1 (the native pseudo-terminal, the main blocker),
> `xterm` 4 → the scoped `@xterm/*` v6 packages, `@electron/remote` 1 → 2, plus
> `electron-builder`, `@electron/rebuild`, `systeminformation`, and `geolite2-redist`.
>
> The name is the original; a distinct fork name is still to be chosen.
...
**Build prerequisites** (needed to compile the native `node-pty` module):
- **Node.js 20+** and a recent npm
- **Python 3** on your `PATH` (for `node-gyp`)
- A C/C++ toolchain: **macOS** → Xcode command line tools · **Linux** → `build-essential` (gcc/make) ·
  **Windows** → the **"Desktop development with C++"** workload from
  [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)
  (the old `windows-build-tools` npm package is no longer used and is not required)
...
on Windows (a normal, non-admin terminal is fine now):
- clone the repository
